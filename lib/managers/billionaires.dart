...
class BillionairesManager extends ChangeNotifier {
  ...
  bool get isLoadingCrypto =>
      _statusCrypto == Status.loading || _statusCrypto == Status.ideal;
  ...
  bool get isTweetLoading =>
      _tweetStatus == Status.loading || _tweetStatus == Status.ideal;
  ...
  bool get isViewLoading =>
      _viewStatus == Status.loading || _viewStatus == Status.ideal;
  ...
  bool get isLoadingBDetail =>
      _statusBDetail == Status.loading || _statusBDetail == Status.ideal;
  ...
  bool get isLoadingCDetail =>
      _statusCDetail == Status.loading || _statusCDetail == Status.ideal;
    ...
    try {
      ...
      if (response.status) {
        try {
          ...
          if (_cryptoDetailRes?.cryptoData?.cryptoSymbol != null &&
              _cryptoDetailRes?.cryptoData?.cryptoSymbol?.isNotEmpty == true) {
            selectedSymbol =
                _cryptoDetailRes?.cryptoData?.cryptoSymbol?.isNotEmpty == true
                    ? _cryptoDetailRes?.cryptoData?.cryptoSymbol
                        ?.firstWhere((symbols) => symbols.symbol == symbol)
                    :
                    // _cryptoDetailRes?.cryptoData?.cryptoSymbol?.first:
                    null;
          }
          if (_cryptoDetailRes?.cryptoData?.rates != null &&
              _cryptoDetailRes?.cryptoData?.rates?.isNotEmpty == true) {
            selectedItem =
                _cryptoDetailRes?.cryptoData?.rates?.isNotEmpty == true
                    ? currency != null
                        ? _cryptoDetailRes?.cryptoData?.rates
                            ?.firstWhere((rate) => rate.currency == currency)
                        : _cryptoDetailRes?.cryptoData?.rates?.first
                    : null;
          }
          usdController.text =
              (((selectedSymbol?.price ?? 0) * (selectedItem?.price ?? 0)) *
  ...
  bool get isLoadingFiat =>
      _statusFiat == Status.loading || _statusFiat == Status.ideal;
  ...
  void calculateWithPriceChange() {
    ...
    btcController.text =
        "${(double.parse(usdController.text) / ((selectedSymbol?.price ?? 0) * (selectedItem?.price ?? 0)))}";
  ...
  Future getCryptoFiat() async {
    ...
    try {
      ...
      if (response.status) {
        ...
        if (_cryptoFiatRes?.cryptoData?.cryptoSymbol != null &&
            _cryptoFiatRes?.cryptoData?.cryptoSymbol?.isNotEmpty == true) {
          selectedSymbol =
              _cryptoFiatRes?.cryptoData?.cryptoSymbol?.isNotEmpty == true
                  ? _cryptoFiatRes?.cryptoData?.cryptoSymbol?.first
                  : null;
        }
        if (_cryptoFiatRes?.cryptoData?.rates != null &&
            _cryptoFiatRes?.cryptoData?.rates?.isNotEmpty == true) {
          selectedItem = _cryptoFiatRes?.cryptoData?.rates?.isNotEmpty == true
              ? _cryptoFiatRes?.cryptoData?.rates?.first
              : null;
        }
        usdController.text =
            (((selectedSymbol?.price ?? 0) * (selectedItem?.price ?? 0)) *
  ...
  Future requestAddToFav(String twitName, {CryptoTweetPost? profiles}) async {
    try {
      Map request = {"twitter_name": twitName};
      ApiResponse response = await apiRequest(
        url: Apis.addFav,
        showProgress: true,
        request: request,
  ...
  Future requestRemoveToFav(String twitName,
      {int? from, CryptoTweetPost? profiles}) async {
    try {
      Map request = {"twitter_name": twitName};
      ApiResponse response = await apiRequest(
        url: Apis.removeFav,
