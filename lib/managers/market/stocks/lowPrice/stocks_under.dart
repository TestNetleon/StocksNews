...
class StocksUnderManager extends ChangeNotifier {
  MarketDataRes? _data;
  ...
  Future getData({loadMore = false, filter = false, required slug}) async {
    final filterRequest =
    ...
    try {
      ...
      final request = filterRequest != null
          ? {"page": "$_page", ...filterRequest, "slug": slug}
          : {"page": "$_page", "slug": slug};

      ApiResponse response = await apiRequest(
        url: Apis.lowPricedStocksUnder,
        request: request,
      );
