...
class ReferralPointsManager extends ChangeNotifier {
  ...
  Future getData({loadMore = false, type}) async {
    if (loadMore == false) {
      _page = 1;
    }
    ...
    try {
      ...
      final request = {"page": "$_page", "txn_type": type ?? ""};
      ApiResponse response = await apiRequest(
        url: Apis.referralTransaction,
        request: request,
      );
      if (response.status) {
        if (_page == 1) {
