...
class TodayBreakoutStockProvider extends ChangeNotifier with AuthProviderBase {
  ...
  Future getData({showProgress = false, loadMore = false}) async {
    ...
    try {
      ...
      if (response.status) {
        _error = null;
        if (_page == 1) {
          _data = breakoutStocksResFromJson(jsonEncode(response.data));
          _extra = response.extra is Extra ? response.extra : null;
        } else {
          _data?.addAll(breakoutStocksResFromJson(jsonEncode(response.data)));
        }
      } else {
        if (_page == 1) {
