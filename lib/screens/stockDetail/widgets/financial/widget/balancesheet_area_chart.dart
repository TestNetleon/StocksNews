...
import 'package:stocks_news_new/screens/stockDetail/widgets/financial/widget/arer_chart_top_widget.dart';
...
class BalanceSheetAreaChart extends StatefulWidget {
  const BalanceSheetAreaChart({super.key});

  @override
  State<BalanceSheetAreaChart> createState() => _BalanceSheetAreaChartState();
}

class _BalanceSheetAreaChartState extends State<BalanceSheetAreaChart> {
  double? minAbsValue;
  double? maxAbsValue;
  dynamic maxPeriod;
  dynamic minPeriod;
  ...
  void initState() {
    ...
    if (data!.length < 10) {
      provider.changeTabAreaBalanceTenInt("1", data);
    } else {
      provider.changeTabAreaBalanceTen("1", data);
    }

    final maxRevenue = provider.areaIncomeChartBalance?.isNotEmpty == true
        ? provider.areaIncomeChartBalance!
            .map((e) => e.totalAssets)
            .reduce((a, b) => a!.abs() > b!.abs() ? a : b)
        : 0;
    final maxNetIncome = provider.areaIncomeChartBalance?.isNotEmpty == true
        ? provider.areaIncomeChartBalance!
            .map((e) => e.totalLiabilities)
            .reduce((a, b) => a!.abs() > b!.abs() ? a : b)
        : 0;
    final maxEbitda = provider.areaIncomeChartBalance?.isNotEmpty == true
        ? provider.areaIncomeChartBalance!
            .map((e) => e.totalEquity)
            .reduce((a, b) => a!.abs() > b!.abs() ? a : b)
        : 0;

    minRevenue = (provider.areaIncomeChartBalance?.isNotEmpty == true &&
            provider.areaIncomeChartBalance != null &&
            provider.areaIncomeChartBalance!
    ...
    minNetIncome = (provider.areaIncomeChartBalance?.isNotEmpty == true &&
            provider.areaIncomeChartBalance != null &&
            provider.areaIncomeChartBalance!.any(
        ...
        ? provider.areaIncomeChartBalance!
            ...
            .reduce(
                (a, b) => a! < b! ? a : b) // Find the minimum negative value
        : null;
    minEbitda = (provider.areaIncomeChartBalance?.isNotEmpty == true &&
            provider.areaIncomeChartBalance != null &&
            provider.areaIncomeChartBalance!
                .any((e) => e.totalEquity != null && e.totalEquity! < 0))
        ? provider.areaIncomeChartBalance!
            .map((e) => e.totalEquity)
            .where((e) => e != null && e < 0) // Filter only negative values
            .reduce(
    ...
    final maxPeriodItem = provider.areaIncomeChartBalance?.isNotEmpty == true
        ? provider.areaIncomeChartBalance!
            .map((e) => int.tryParse(e.period) ?? 0)
            .reduce((a, b) => a.abs() > b.abs() ? a : b)
        : 0;

    final minPeriodItem = provider.areaIncomeChartBalance?.isNotEmpty == true
        ? provider.areaIncomeChartBalance!
            .map((e) => int.tryParse(e.period) ?? 0)
            .reduce((a, b) => a.abs() < b.abs() ? a : b)
        : 0;
  ...
  Widget build(BuildContext context) {
    ...
    List<Chart>? financialData =
        data != null ? provider.areaIncomeChartBalance : [];
            ...
            children: [
              GestureDetector(
                onTap: () {
                  ...
                  if (data!.length < 10) {
                    ...
                    provider.changeTabAreaBalanceTen("1", data);
                    final maxPeriodItem =
                        provider.areaIncomeChartBalance?.isNotEmpty == true
                            ? provider.areaIncomeChartBalance!
                                .map((e) => int.tryParse(e.period) ?? 0)
                                .reduce((a, b) => a.abs() > b.abs() ? a : b)
                            : 0;
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: provider.changeTabAreaValueBalance == "1"
                        ? ThemeColors.accent
                        : Colors.transparent,
                    borderRadius: const BorderRadius.all(Radius.circular(5)),
                  ),
              ...
              GestureDetector(
                onTap: () {
                  ...
                  if (data!.length < 6) {
                    provider.changeTabAreaBalanceTenInt("0", data);
                    final maxPeriodItem = data?.isNotEmpty == true
                        ? data!
                            .map((e) => int.tryParse(e.period) ?? 0)
                            .reduce((a, b) => a.abs() > b.abs() ? a : b)
                    ...
                    provider.changeTabAreaBalanceSix("0", data);
                    final maxPeriodItem =
                        provider.areaIncomeChartBalance?.isNotEmpty == true
                            ? provider.areaIncomeChartBalance!
                                .map((e) => int.tryParse(e.period) ?? 0)
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: provider.changeTabAreaValueBalance == "0"
                        ? ThemeColors.accent
                        : Colors.transparent,
                    borderRadius: const BorderRadius.all(Radius.circular(5)),
                  ),
  ...
  Widget bottomTitles(double value, TitleMeta meta) {
    ...
    List<Chart> dataChart = provider.areaIncomeChartBalance ?? [];
    for (var i = 0; i < dataChart.length; i++) {
      if (value ==
          double.parse("${provider.areaIncomeChartBalance?[i].period}")) {
        return SideTitleWidget(
            axisSide: meta.axisSide,
            space: 20, // margin top
            child: Text(
              "${provider.areaIncomeChartBalance?[i].period}",
