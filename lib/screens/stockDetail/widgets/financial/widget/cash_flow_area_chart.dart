...
class CashFlowAreaChart extends StatefulWidget {
  const CashFlowAreaChart({super.key});
...
class _CashFlowAreaChartState extends State<CashFlowAreaChart> {
  ...
  void initState() {
    ...
    if (data!.length < 10) {
      provider.changeTabAreaCashFlowTenInt("1", data);
    } else {
      provider.changeTabAreaCashFlowTen("1", data);
    }

    maxRevenue = provider.areaIncomeChartCashFlow?.isNotEmpty == true
        ? provider.areaIncomeChartCashFlow!
            .map((e) => e.operatingCashFlow1)
            .reduce((a, b) => a!.abs() > b!.abs() ? a : b)
        : 0;
    maxNetIncome = provider.areaIncomeChartCashFlow?.isNotEmpty == true
        ? provider.areaIncomeChartCashFlow!
            .map((e) => e.operatingCashFlow2)
            .reduce((a, b) => a!.abs() > b!.abs() ? a : b)
        : 0;
    maxEbitda = provider.areaIncomeChartCashFlow?.isNotEmpty == true
        ? provider.areaIncomeChartCashFlow!
            .map((e) => e.operatingCashFlow3)
            .reduce((a, b) => a!.abs() > b!.abs() ? a : b)
        : 0;

    minRevenue = (provider.areaIncomeChartCashFlow?.isNotEmpty == true &&
            provider.areaIncomeChartCashFlow != null &&
            provider.areaIncomeChartCashFlow!.any((e) =>
    ...
    minNetIncome = (provider.areaIncomeChartCashFlow?.isNotEmpty == true &&
            provider.areaIncomeChartCashFlow != null &&
            provider.areaIncomeChartCashFlow!.any((e) =>
        ...
        ? provider.areaIncomeChartCashFlow!
            ...
            .reduce(
                (a, b) => a! < b! ? a : b) // Find the minimum negative value
        : null;
    minEbitda = (provider.areaIncomeChartCashFlow?.isNotEmpty == true &&
            provider.areaIncomeChartCashFlow != null &&
            provider.areaIncomeChartCashFlow!.any((e) =>
    ...
    final minPeriodItem = provider.areaIncomeChartCashFlow?.isNotEmpty == true
        ? provider.areaIncomeChartCashFlow!
            .map((e) => int.tryParse(e.period) ?? 0)
            .reduce((a, b) => a.abs() < b.abs() ? a : b)
        : 0;
    ...
    setState(() {});
    Utils().showLog('Error initializing Firebase12: $maxNetIncome');
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<FlSpot> generateSpotsEbitda(List<Chart>? financialData) {
      return financialData!
          .map((data) => FlSpot(double.parse("${data.period}"),
              double.parse("${data.operatingCashFlow3}")))
          .toList();
    }
            ...
            children: [
              GestureDetector(
                onTap: () {
                  ...
                  if (data!.length < 10) {
                    ...
                    provider.changeTabAreaCashFlowTen("1", data);
                    final maxPeriodItem =
                        provider.areaIncomeChartCashFlow?.isNotEmpty == true
                            ? provider.areaIncomeChartCashFlow!
                                .map((e) => int.tryParse(e.period) ?? 0)
                                .reduce((a, b) => a.abs() > b.abs() ? a : b)
                            : 0;
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: provider.changeTabAreaValueCashFlow == "1"
                        ? ThemeColors.accent
                        : Colors.transparent,
                    borderRadius: const BorderRadius.all(Radius.circular(5)),
                  ),
              ...
              GestureDetector(
                onTap: () {
                  ...
                  if (data!.length < 6) {
                    provider.changeTabAreaCashFlowTenInt("0", data);
                    final maxPeriodItem = data?.isNotEmpty == true
                        ? data!
                            .map((e) => int.tryParse(e.period) ?? 0)
                            .reduce((a, b) => a.abs() > b.abs() ? a : b)
                    ...
                    provider.changeTabAreaCashFlowSix("0", data);
                    final maxPeriodItem =
                        provider.areaIncomeChartCashFlow?.isNotEmpty == true
                            ? provider.areaIncomeChartCashFlow!
                                .map((e) => int.tryParse(e.period) ?? 0)
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: provider.changeTabAreaValueCashFlow == "0"
                        ? ThemeColors.accent
                        : Colors.transparent,
                    borderRadius: const BorderRadius.all(Radius.circular(5)),
                  ),
  ...
  Widget bottomTitles(double value, TitleMeta meta) {
    ...
    List<Chart> dataChart = provider.areaIncomeChartCashFlow ?? [];
    for (var i = 0; i < dataChart.length; i++) {
      if (value ==
          double.parse("${provider.areaIncomeChartCashFlow?[i].period}")) {
        return SideTitleWidget(
            axisSide: meta.axisSide,
            space: 20, // margin top
            child: Text(
              "${provider.areaIncomeChartCashFlow?[i].period}",
