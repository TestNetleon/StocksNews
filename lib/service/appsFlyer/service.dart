...
class AppsFlyerService {
    ...
    try {
      ...
      try {
        _appsFlyerSdk?.onAppOpenAttribution((data) {
          Utils().showLog('onAppOpenAttribution=> $data');
          appsflyerExtra1 = jsonEncode(data);
        });
      } catch (e) {
        //
      }
  ...
  handleDeepLinking() {
    _appsFlyerSdk?.onDeepLinking(
      (result) {
        ...
        Utils().showLog('DATA RESULT =>${jsonEncode(result)}');

        try {
          appsflyerExtra = jsonEncode(result);
        } catch (e) {
          //
        }
        String? deepLinkSub1;
        ...
        if (result.deepLink?.deepLinkValue != null &&
  ...
  String? getDistributorCode(String input) {
    ...
    if (!keyValuePairs.containsKey("distributor_code")) {
      return null;
    }

    String? memCode = keyValuePairs.remove("distributor_code");

    String formattedParams = keyValuePairs.entries
        .map((entry) => "${entry.key}=${entry.value}")
        .join("&");
    Utils().showLog('Distributor Data ${"$memCode&$formattedParams"}');
    return "$memCode&$formattedParams";
  }
