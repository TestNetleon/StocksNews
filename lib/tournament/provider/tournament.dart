...
//MARK: TIMER
  ...
  void _startTimer(Duration initialDuration, {required bool isStart}) {
    ...
    if (isStart) {
      ...
      _remainingTime =
          _detailRes?.battleTime?.startTime!.difference(DateTime.now());
    } else {
      _totalDuration = _detailRes?.battleTime?.endTime!
          .difference(_detailRes?.battleTime!.currentTime ?? DateTime.now());
      _remainingTime =
          _detailRes?.battleTime?.endTime!.difference(DateTime.now());
    }
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      if (hours == 0 && minutes == 0 && seconds == 0) {
        ...
        if (seconds > 0) {
          seconds--;
          int elapsedSeconds =
              _totalDuration!.inSeconds - _remainingTime!.inSeconds;
