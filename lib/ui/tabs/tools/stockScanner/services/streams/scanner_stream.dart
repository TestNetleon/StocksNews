...
class MarketScannerStream {
  ...
  Future<void> initializePorts() async {
    ...
    final provider = navigatorKey.currentContext!.read<MarketScannerM>();
  ...
  Future<void> _connectToSseClient(
      String url, MarketScannerM provider) async {
    try {
  ...
  void _handleEventData(String data, MarketScannerM provider) {
    try {
      _isOfflineCalled = false;
